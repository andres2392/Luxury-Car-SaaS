...
def list_dealers(db: Annotated[Session, Depends(get_db)]) -> list[DealerResponse]:
    return get_dealers(db)


@router.get("/{dealer_id}", response_model=DealerResponse)
def get_dealer(dealer_id: int, db: Annotated[Session, Depends(get_db)]) -> DealerResponse:
